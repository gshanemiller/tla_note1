...
procedure SpecialOp(k_in1, v_in1, op_in1)
begin
special_op_begin:
    print "this code won't be hit";
    return;
end procedure;
...
\* BEGIN TRANSLATION (chksum(pcal) = "7e89e5af" /\ chksum(tla) = "aa5f21be") PCal-782ae353228cc15170c4590ec7433f1f
CONSTANT defaultInitValue
VARIABLES Hash, pc, stack, k_in, v_in, op_in, k_in1, v_in1, op_in1

vars == << Hash, pc, stack, k_in, v_in, op_in, k_in1, v_in1, op_in1 >>
...
Init == (* Global variables *)
        ...
        (* Procedure SpecialOp *)
        /\ k_in1 = [ self \in ProcSet |-> defaultInitValue]
        /\ v_in1 = [ self \in ProcSet |-> defaultInitValue]
        /\ op_in1 = [ self \in ProcSet |-> defaultInitValue]
...
perform_op_begin(self) == /\ pc[self] = "perform_op_begin"
                          ...
                          /\ UNCHANGED << k_in1, v_in1, op_in1 >>

PerformOp(self) == perform_op_begin(self)

special_op_begin(self) == /\ pc[self] = "special_op_begin"
                          /\ PrintT("this code won't be hit")
                          /\ pc' = [pc EXCEPT ![self] = Head(stack[self]).pc]
                          /\ k_in1' = [k_in1 EXCEPT ![self] = Head(stack[self]).k_in1]
                          /\ v_in1' = [v_in1 EXCEPT ![self] = Head(stack[self]).v_in1]
                          /\ op_in1' = [op_in1 EXCEPT ![self] = Head(stack[self]).op_in1]
                          /\ stack' = [stack EXCEPT ![self] = Tail(stack[self])]
                          /\ UNCHANGED << Hash, k_in, v_in, op_in >>

SpecialOp(self) == special_op_begin(self)
...
worker_begin(self) == /\ pc[self] = "worker_begin"
                      /\ \E k \in HashKey:
                           \E v \in HashValue:
                             \E op \in ClientOps:
                               IF k=1000
                                  THEN /\ /\ k_in1' = [k_in1 EXCEPT ![self] = k]
                                          /\ op_in1' = [op_in1 EXCEPT ![self] = op]
                                          /\ stack' = [stack EXCEPT ![self] = << [ procedure |->  "SpecialOp",
                                                                                   pc        |->  "worker_begin",
                                                                                   k_in1     |->  k_in1[self],
                                                                                   v_in1     |->  v_in1[self],
                                                                                   op_in1    |->  op_in1[self] ] >>
                                                                               \o stack[self]]
                                          /\ v_in1' = [v_in1 EXCEPT ![self] = v]
                                       /\ pc' = [pc EXCEPT ![self] = "special_op_begin"]
                                       /\ UNCHANGED << k_in, v_in, op_in >>
                                  ELSE /\ /\ k_in' = [k_in EXCEPT ![self] = k]
...
Next == (\E self \in ProcSet: PerformOp(self) \/ SpecialOp(self))
           \/ (\E self \in 1..1: Worker(self))
           \/ Terminating

Spec == /\ Init /\ [][Next]_vars
        /\ \A self \in 1..1 : /\ WF_vars(Worker(self))
                              /\ WF_vars(SpecialOp(self))
                              /\ WF_vars(PerformOp(self))
